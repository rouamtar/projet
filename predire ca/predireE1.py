...
try:
    model = joblib.load(r'C:\Users\User\Desktop\algo_ml\predire ca\random_forest_model.joblib')
...
@app.route('/')
def home():
    return render_template('predireE1.html')

@app.route('/predireE1', methods=['POST'])
def predict():
    try:
        ...
        return render_template('predireE1.html', prediction_text=f"Le chiffre d'affaires prédit est : {prediction[0]} TND")
    except Exception as e:
        return render_template('predireE1.html', error_message=f"Une erreur s'est produite : {e}")

if __name__ == '__main__':
    app.run(debug=True, port=8000)  # Vous pouvez choisir un autre port