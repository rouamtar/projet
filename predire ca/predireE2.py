...
try:
    model = joblib.load(r'C:\Users\User\Desktop\algo_ml\predire ca\random_forest_model2.joblib')
...
@app.route('/')
def home():
    return render_template('predireE2.html')

@app.route('/predireE2', methods=['POST'])
def predict():
    try:
        ...
        return render_template('predireE2.html', prediction_text=f"Le chiffre d'affaires prédit est : {prediction[0]} TND")
    except Exception as e:
        return render_template('predireE2.html', error_message=f"Une erreur s'est produite : {e}")

if __name__ == '__main__':
    app.run(debug=True, port=8001)  # Vous pouvez choisir un autre port